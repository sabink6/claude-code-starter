...
- Merge results into:
  ...
  2. Accessibility findings — reproduce a11y-reviewer's report as-is, in its
     own Critical/Serious/Moderate/Minor scale (do not relabel or force a
     shared vocabulary between the two agents; each keeps its own scale)
  3. Code quality findings — reproduce code-quality-reviewer's report as-is,
     in its own Critical/Serious/Moderate/Minor scale
  4. Combined action plan (ordered checklist, pulling the Suggested
     fix/Recommended fix snippets each agent already provided)
  5. Questions/uncertainties (anything that needs human intent, including
     each agent's own Manual Verification Required items)
